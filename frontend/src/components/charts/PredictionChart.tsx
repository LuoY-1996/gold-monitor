...
export default function PredictionChart({
  ...
  useEffect(() => {
    ...
    if (prediction && prediction.predicted_price_7d) {
      ...
      const forecastX = lastX + chartW * 0.08; // shift right
    ...
    setDrawn(true);
  }, [historicalData, prediction, height]);
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', position: 'relative' }}>
      ...
      {!drawn && <div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Spin /></div>}
    </div>
  );
}
